...
class Guest_db(db_operation):
    def _show(self):
        return self._Select(self._GUEST)

    def Create(self, id, password, email,course):
        query =  "INSERT into " + self._GUEST +" (ID, PASSWORD, EMAIL, COURSE_SEMESTER) values ("+ id + ",'" + password + "','"+ email + "','"+ course +"')"
        print(query)
        return self._db_cursor(query)
        
    def Delete(self, id):
        query = "DELETE FROM " +  self._GUEST + " WHERE ID = " + str(id)
        return self._db_cursor(query)
        
    def Get(self,id):
        query = "SELECT *  from " + self._GUEST + " WHERE ID = " + str(id)
        n = self._db_select(query)
        if n == []:
        #can't find this name: return 1
